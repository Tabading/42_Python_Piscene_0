...
# width			: (total terminal width) - 5(100%|) -  (| (333)/(333) ) - 26([00:01<00:00, 177.24it/s](\n))
...
def ft_tqdm(lst: range) -> None:
	total = len(lst)
	width = os.get_terminal_size().columns - 5 - (4 + (2 * len(str(total)))) - 26
...
# def ft_tqdm(lst: range) -> None:
# 	total = len(lst)
# 	width = os.get_terminal_size().columns - 5 - (4 + (2 * len(str(total)))) - 26 - 3
# 	for i, elem in enumerate(lst, 1):
# 		p = i * 100 // total
# 		bar = "=" * (p * width // 100)
# 		print(f"\r{p:3}%|[{bar + '>':<{width}}]| {i}/{total}", end="", flush=True)
